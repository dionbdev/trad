...
router.post("/register", middleware.sanitizeHtml, function(req, res){
    ...
    User.register(newUser, req.body.password, function(err, user){
        ...
        passport.authenticate("local")(req, res, function(){
           req.flash("success", "Welcome to Trad " + user.username);
           res.redirect("/tally"); 
        });
    });
});
...
// My Tallys I know this is not correct spelling
router.get("/users/:id", function(req, res){
  User.findById(req.params.id, function(err, foundUser){
    if(err){
      req.flash("error", "This ain't working");
      res.direct("/");
    }
    Tally.find().where('author.id').equals(foundUser._id).exec(function(err, tally){
      if(err){
        req.flash("error", "This ain't working");
        res.direct("/");
      }
      res.render("users/show", {user: foundUser, tally: tally});
    })
  });
});
  ...
  async.waterfall([
    ...
    function(token, user, done) {
      var smtpTransport = nodemailer.createTransport({
        service: 'Gmail', 
        auth: {
          user: 'tradtally@gmail.com',
          pass: process.env.GMAILPW
        }
      });
      var mailOptions = {
        to: user.email,
        from: 'tradtally@gmail.com',
